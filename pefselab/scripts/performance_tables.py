...
if __name__ == "__main__":
    ...
    common_codes = (
        set(bilty_table.keys()) & set(efselab_table.keys()) & set(udpipe_table.keys())
    )
    union_codes = (
        set(bilty_table.keys()) | set(efselab_table.keys()) | set(udpipe_table.keys())
    )
    table = sorted(
        [
            (code, bilty_table[code], udpipe_table[code], efselab_table[code])
    ...
    for col1, col2 in zip(
        table[: round(len(table) / 2)], table[round(len(table) / 2) :]
    ):
    ...
    print(
        (
            r"\multicolumn{8}{c}{Summary statistics below refer to all %d "
            r"treebanks, in both columns above} \\"
        )
        % len(common_codes)
    )
    print(r"\midrule")
    print(
        r"Mean error rate & %s & No.\ of (shared) top ranks & %s \\"
        % (
    ...
    print(
        "%d treebanks in %d languages evaluated" % (len(common_codes), len(languages)),
        file=sys.stderr,
    )

    print(
        "bilty is missing: %s" % " ".join(union_codes - set(bilty_table.keys())),
        file=sys.stderr,
    )
    print(
        "udpipe is missing: %s" % " ".join(union_codes - set(udpipe_table.keys())),
        file=sys.stderr,
    )
    print(
        "efselab is missing: %s" % " ".join(union_codes - set(efselab_table.keys())),
        file=sys.stderr,
    )
    ...
    bilty_ax = plt.scatter(
        train_sizes,
        bilty_acc,
        marker="o",
        edgecolors="b",
        facecolors=["b" if x <= y else "none" for x, y in zip(bilty_acc, efselab_acc)],
    )
    efselab_ax = plt.scatter(
        train_sizes,
        efselab_acc,
        marker="s",
        edgecolors="r",
        facecolors=["r" if x >= y else "none" for x, y in zip(bilty_acc, efselab_acc)],
    )
    ...
    print(r"bilty: e = %.2f \cdot n^{%.2f})" % (np.power(10, b), a), file=sys.stderr)
    ...
    print(r"efselab: e = %.2f \cdot n^{%.2f})" % (np.power(10, b), a), file=sys.stderr)
