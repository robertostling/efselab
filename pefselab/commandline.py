...
def create_parser():
    ...
    parser.add_option(
        "--skip-tokenization",
        dest="skip_tokenization",
        action="store_true",
        help="Assume tokenized input",
    )
    parser.add_option(
        "--skip-segmentation",
        dest="skip_segmentation",
        action="store_true",
        help="Assume one sentence per line",
    )
    parser.add_option(
        "--tokenized",
        dest="tokenized",
    ...
    parser.add_option(
        "-u",
        "--ud-tagging-model",
        dest="ud_tagging_model",
        default=UD_TAGGING_MODEL,
        metavar="FILENAME",
    ...
    parser.add_option(
        "-l",
        "--lemmatization-model",
        dest="lemmatization_model",
        default=LEMMATIZATION_MODEL,
        metavar="MODEL",
...
def validate_options(options, args):
    ...
    if options.lemmatized and not os.path.exists(options.lemmatization_model):
        sys.exit(
            ERROR_MESSAGES.not_found_lemmatizer_model % options.lemmatization_model
        )
    ...
    if options.parsed and not os.path.exists(options.parsing_model + ".mco"):
        sys.exit(
            ERROR_MESSAGES.not_found_parsing_model % (options.parsing_model + ".mco")
        )
