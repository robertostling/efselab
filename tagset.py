...
''' % (self.name, self.c_n_tags, self.c_tag_str, self.name+'_from_str'))
    ...
    def _subset_compute(self):
        ...
        self._frozen = True
        self._subset_idx = { subset: i
                for i,subset in enumerate(sorted(self.subsets)) }
        self._indexed_subsets = [index_list(xs) for xs in sorted(self.subsets)]
        self._subset_n_bits = [math.ceil(math.log2(len(set(trans))))
                              for trans in self._indexed_subsets]
        self._subset_shifts = list(
                itertools.accumulate([0] + self._subset_n_bits))[:-1]
    ...
    def _subset_table(self):
        ...
        table = [sum(x << b for b,x in zip(self._subset_shifts, xs))
        ...
        return 'static const %s %s[%s] = {\n%s\n};\n\n' % (
                dtype, self.c_tag_subsets, self.c_n_tags, body)
...
class TransformedTag(Tag):
    ...
    def c_value(self):
        return '(%s[%s] & 0x%x)' % (
            self.tagset.c_tag_subsets, self.tag.c_value(),
